...
uint16_t TurboAnalogRead::lastMeasurement = 0;
uint8_t TurboAnalogRead::flag = 0;
void (*TurboAnalogRead::interruptFunction)() = NULL;
...
void TurboAnalogRead::Setup(uint8_t pin, void (*ISRFunc)()){
...
void TurboAnalogRead::Setup_timer(uint8_t pin, void (*ISRFunc)()){
    Setup(pin, ISRFunc);
...
void TurboAnalogRead::Measure(){
    ADCSRA |= (1 << ADSC); //Starter konvertering
}

ISR(ADC_vect){

    TurboAnalogRead::lastMeasurement = ADC;
    TurboAnalogRead::flag += 1;
    
    if(TurboAnalogRead::interruptFunction != NULL){
        (*TurboAnalogRead::interruptFunction)(); //den funktion der skal kaldes.
    }
